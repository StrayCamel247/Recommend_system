...
class dat_api():
    ...
    #try set connection with dat files
        try:
            ...
            # bookmarks'header is ['id','md5','title','url','md5Principal','urlPrincipal']
            ...
            for val in self.bookmarks['title'].str.split(' +'):
                for i in val:
                    if i not in genres_set:
                        ...
                        genres_set[i]+=1
            data = pd.DataFrame({'id': [i for i in range(len(genres_set)) ],'word': [val for val in genres_set.keys()],'counts': [val for val in genres_set.values()]})
            print(data)
            # 写入文件/save/bookmarks.dat  运行一次就好了
            data.to_csv(os.path.dirname(__file__)+'/save/bags_of_title.dat', sep=' ', header=True, index=True)
