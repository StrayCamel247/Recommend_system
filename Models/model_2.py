...
def pre_blurb():
    ...
    print('blurb.shape = ', blurb.shape)
    return blurb
blurb = pre_blurb()
mode = {'model_2': blurb}
train_features, train_labels, val_features, val_lables, test_features, test_lables = get_train_val_test(origin_DATA.features.values, origin_DATA.labels.values, model=mode)
# 嵌入矩阵的维度
embed_dim = 4
embed_dim_title = 16
...
def book_emded_layer(b_isbn, b_title, b_atuhor, b_year, b_publisher, b_blurb):
...
#     u_id_layer_drop = keras.layers.Dropout(rate=0.5, name='u_id_layer_drop')(u_id_layer)
    ...
    u_id_reshape = keras.layers.Reshape([32], name='u_id_reshape')(u_id_layer)
...
def get_book_feature(b_isbn_embedd, b_author_embedd, b_year_embedd, b_publisher_embedd, b_title_embedd, b_blurb_embedd):
    ...
    # 将title和blurb合并
    b_title_reshape = keras.layers.Reshape([b_title_pool.shape[3]], name='b_title_reshape')(b_title_pool)
...
class Net_works():
    ...
    def creat_model(self):
        ...
        book_isbn_embedd, book_author_embedd, book_year_embedd, book_publisher_embedd, book_title_embedd, book_blurb_embedd = book_emded_layer(book_isbn, book_title, book_author, book_year, book_publisher,  book_blurb)
